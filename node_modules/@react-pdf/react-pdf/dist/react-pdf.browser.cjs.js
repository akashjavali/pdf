...
var standardFonts = ['Courier', 'Courier-Bold', 'Courier-Oblique', 'Courier-BoldOblique', 'Helvetica', 'Helvetica-Bold', 'Helvetica-Oblique', 'Helvetica-BoldOblique', 'Times-Roman', 'Times-Bold', 'Times-Italic', 'Times-BoldItalic', 'Symbol', 'ZapfDingbats'];
...
const register = (src, _ref) => {
  let family = _ref.family,
      otherOptions = objectWithoutProperties(_ref, ['family']);
...
const load = (() => {
  var _ref2 = asyncToGenerator(function* (fontFamily, doc) {
    ...
    if (font && !font.data && !font.loading) {
      ...
      if (isUrl(font.src)) {
        const data = yield fetchFont(font.src);
...
const getWords = glyphString => {
  const words = [];
  const start = glyphString.start;

  let lastIndex = 0;

  for (var _iterator = glyphString, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
    var _ref2;
...
var lineBreaker = (({ hyphenationCallback } = {}) => Textkit => {
  ...
  return class KPLineBreaker {
    ...
    measureWidth(glyphString) {
      ...
      const font = _glyphString$glyphRun.font,
...
const fetchImage = src => {
  if (typeof src === 'object') {
    if (src.data && src.format) {
      // Local file given
      return new Promise((resolve, reject) => resolve(getImage(src.data, src.format)));
    }
...
const getCodePoints = string => Array.from(string).map(char => char.codePointAt(0).toString(16)).join('-');

const buildEmojiUrl = emoji => {
  var _Font$getEmojiSource = Font.getEmojiSource();

  const url = _Font$getEmojiSource.url,
...
class Document$2 {
  ...
  loadFonts() {
    ...
    return asyncToGenerator(function* () {
      const promises = [];
      const listToExplore = _this.children.slice(0);

      while (listToExplore.length > 0) {
        const node = listToExplore.shift();

        if (node.style && node.style.fontFamily) {
          promises.push(Font.load(node.style.fontFamily, _this.root));
        }
  ...
  loadEmojis() {
    ...
    return asyncToGenerator(function* () {
      const promises = [];
      const listToExplore = _this2.children.slice(0);
  ...
  loadImages() {
    ...
    return asyncToGenerator(function* () {
      const promises = [];
      const listToExplore = _this3.children.slice(0);
  ...
  loadAssets() {
    ...
    return asyncToGenerator(function* () {
      yield Promise.all([_this4.loadFonts(), _this4.loadImages()]);
    })();
  }
  ...
  wrapChildren() {
    ...
    return asyncToGenerator(function* () {
      for (let i = 0; i < _this5.children.length; i++) {
        yield _this5.children[i].wrapPage();
      }
    })();
  }
  ...
  renderChildren() {
    ...
    return asyncToGenerator(function* () {
      for (let i = 0; i < _this6.children.length; i++) {
        yield _this6.children[i].render();
      }
    })();
  }
  ...
  render() {
    ...
    return asyncToGenerator(function* () {
      try {
        _this7.addMetaData();
        _this7.applyProps();
        yield _this7.loadEmojis();
        yield _this7.loadAssets();
        yield _this7.wrapChildren();
        yield _this7.renderChildren();
        _this7.root.end();
        Font.reset();
      } catch (e) {
        throw e;
...
const Borders = {
  ...
  drawBorders() {
    ...
    const left = _getAbsoluteLayout.left,
    ...
    const borderTopWidth = _getComputedStyles$bo === undefined ? 0 : _getComputedStyles$bo;
    var _getComputedStyles$bo2 = _getComputedStyles.borderRightWidth;
    const borderRightWidth = _getComputedStyles$bo2 === undefined ? 0 : _getComputedStyles$bo2;
    var _getComputedStyles$bo3 = _getComputedStyles.borderBottomWidth;
    const borderBottomWidth = _getComputedStyles$bo3 === undefined ? 0 : _getComputedStyles$bo3;
    var _getComputedStyles$bo4 = _getComputedStyles.borderLeftWidth;
    const borderLeftWidth = _getComputedStyles$bo4 === undefined ? 0 : _getComputedStyles$bo4;
    var _getComputedStyles$bo5 = _getComputedStyles.borderTopLeftRadius;
    const borderTopLeftRadius = _getComputedStyles$bo5 === undefined ? 0 : _getComputedStyles$bo5;
    var _getComputedStyles$bo6 = _getComputedStyles.borderTopRightRadius;
    const borderTopRightRadius = _getComputedStyles$bo6 === undefined ? 0 : _getComputedStyles$bo6;
    var _getComputedStyles$bo7 = _getComputedStyles.borderBottomRightRadius;
    const borderBottomRightRadius = _getComputedStyles$bo7 === undefined ? 0 : _getComputedStyles$bo7;
    var _getComputedStyles$bo8 = _getComputedStyles.borderBottomLeftRadius;
    const borderBottomLeftRadius = _getComputedStyles$bo8 === undefined ? 0 : _getComputedStyles$bo8;
    var _getComputedStyles$bo9 = _getComputedStyles.borderTopColor;
    const borderTopColor = _getComputedStyles$bo9 === undefined ? 'black' : _getComputedStyles$bo9;
    var _getComputedStyles$bo10 = _getComputedStyles.borderRightColor;
    const borderRightColor = _getComputedStyles$bo10 === undefined ? 'black' : _getComputedStyles$bo10;
    var _getComputedStyles$bo11 = _getComputedStyles.borderBottomColor;
    const borderBottomColor = _getComputedStyles$bo11 === undefined ? 'black' : _getComputedStyles$bo11;
    var _getComputedStyles$bo12 = _getComputedStyles.borderLeftColor;
    const borderLeftColor = _getComputedStyles$bo12 === undefined ? 'black' : _getComputedStyles$bo12;
    var _getComputedStyles$bo13 = _getComputedStyles.borderTopStyle;
    const borderTopStyle = _getComputedStyles$bo13 === undefined ? 'solid' : _getComputedStyles$bo13;
    var _getComputedStyles$bo14 = _getComputedStyles.borderRightStyle;
    const borderRightStyle = _getComputedStyles$bo14 === undefined ? 'solid' : _getComputedStyles$bo14;
    var _getComputedStyles$bo15 = _getComputedStyles.borderBottomStyle;
    const borderBottomStyle = _getComputedStyles$bo15 === undefined ? 'solid' : _getComputedStyles$bo15;
    var _getComputedStyles$bo16 = _getComputedStyles.borderLeftStyle;
    const borderLeftStyle = _getComputedStyles$bo16 === undefined ? 'solid' : _getComputedStyles$bo16;
...
class Base extends Node$1 {
  ...
  drawBackgroundColor() {
    ...
    const backgroundColor = _getComputedStyles.backgroundColor;


    if (backgroundColor) {
      this.root.fillColor(backgroundColor).roundedRect(left + margin.left, top + margin.top, width - margin.left - margin.right, height - margin.top - margin.bottom, 5).fill();
    }
  }
  ...
  renderChildren(page) {
    ...
    return asyncToGenerator(function* () {
      const absoluteChilds = _this.children.filter(function (child) {
        return child.isAbsolute();
      });
      const nonAbsoluteChilds = _this.children.filter(function (child) {
        return !child.isAbsolute();
      });

      for (let i = 0; i < nonAbsoluteChilds.length; i++) {
        yield nonAbsoluteChilds[i].render(page);
      }

      for (let i = 0; i < absoluteChilds.length; i++) {
        yield absoluteChilds[i].render(page);
      }
    })();
  }
}
...
class SubPage extends Base {
  ...
  wrap(height) {
    ...
    for (let i = 0; i < this.children.length; i++) {
      ...
      const fixed = _child$props.fixed,
  ...
  callChildFunctions() {
    ...
    while (listToExplore.length > 0) {
      const node = listToExplore.shift();
      const pageCount = this.page.document.pageCount;
  ...
  render(page) {
    ...
    return asyncToGenerator(function* () {
      _this.root.addPage({
        size: _this.size,
        layout: _this.props.orientation,
        margin: 0
      });

      _this.callChildFunctions();
      _this.layoutFixedElements();

      if (_this.style.backgroundColor) {
        _this.root.fillColor(_this.style.backgroundColor).rect(0, 0, _this.root.page.width, _this.root.page.height).fill();
      }
      ...
      yield _this.renderChildren(page);

      _this.page.renderRuler();
    })();
  }
}
...
const range = (max, steps) => Array.from({ length: Math.ceil(max / steps) }, (_, i) => i * steps);
...
class Page$1 {
  ...
  applyProps() {
    ...
    const paddingTop = _style$paddingTop === undefined ? 0 : _style$paddingTop;
    var _style$paddingLeft = _style.paddingLeft;
    const paddingLeft = _style$paddingLeft === undefined ? 0 : _style$paddingLeft;
  ...
  wrapPage() {
    ...
    return asyncToGenerator(function* () {
      var _style2 = _this.style,
          _style2$paddingTop = _style2.paddingTop;
      const paddingTop = _style2$paddingTop === undefined ? 0 : _style2$paddingTop;
      var _style2$paddingBottom = _style2.paddingBottom;
      const paddingBottom = _style2$paddingBottom === undefined ? 0 : _style2$paddingBottom;

      const height = _this.height - paddingTop - paddingBottom;
      let nextSubpage = _this.initialSubpage.wrap(height);

      while (_this.props.wrap && !nextSubpage.isEmpty()) {
        _this.children.push(nextSubpage);
  ...
  render() {
    ...
    return asyncToGenerator(function* () {
      for (let i = 0; i < _this2.children.length; i++) {
        yield _this2.children[i].render(_this2);
      }
    })();
  }
}
...
Object.assign(Page$1.prototype, Ruler);
...
class TextEngine {
  ...
  get attributedString() {
    ...
    const color = _element$getComputedS2 === undefined ? 'black' : _element$getComputedS2;
    var _element$getComputedS3 = _element$getComputedS.fontFamily;
    const fontFamily = _element$getComputedS3 === undefined ? 'Helvetica' : _element$getComputedS3;
    var _element$getComputedS4 = _element$getComputedS.fontSize;
    const fontSize = _element$getComputedS4 === undefined ? 18 : _element$getComputedS4;
    var _element$getComputedS5 = _element$getComputedS.textAlign;
    const textAlign = _element$getComputedS5 === undefined ? 'left' : _element$getComputedS5,
    ...
    for (var _iterator = this.preprocessors, _isArray = Array.isArray(_iterator), _i = 0, _iterator = _isArray ? _iterator : _iterator[Symbol.iterator]();;) {
      var _ref;
  ...
  layout(width, dirty) {
    ...
    const path = new textkitCore.Path().rect(0, 0, width, INFINITY);
  ...
  render() {
    ...
    const top = _element$getAbsoluteL.top,
...
class Image$1 extends Base {
  ...
  fetch() {
    ...
    return asyncToGenerator(function* () {
      try {
        _this.image = yield fetchImage(_this.props.src);
      } catch (e) {
        _this.image = { width: 0, height: 0 };
  ...
  render() {
    ...
    return asyncToGenerator(function* () {
      const margin = _this2.margin;
      const padding = _this2.padding;

      var _getAbsoluteLayout = _this2.getAbsoluteLayout();

      const left = _getAbsoluteLayout.left,
            top = _getAbsoluteLayout.top;


      _this2.drawBackgroundColor();
      _this2.drawBorders();
      ...
      if (_this2.image.data) {
        // Inner offset between yoga node and image box
        // Makes image centered inside Yoga node
        const containerWidth = _this2.width - margin.right - margin.left;
        const containerHeight = _this2.height - margin.top - margin.bottom;
        const imageWidth = Math.min(containerHeight * _this2.ratio, containerWidth);
        const xOffset = Math.max((containerWidth - imageWidth) / 2, 0);

        _this2.root.image(_this2.image.data, left + padding.left + margin.left + xOffset, top + padding.top + margin.top, {
...
const pdf = input => {
  let parse = (() => {
    var _ref = asyncToGenerator(function* (input) {
  ...
  let toString = (() => {
    var _ref4 = asyncToGenerator(function* () {
      let result = '';
      const render = yield parse(input);
...
class Document$$1 extends React.Component {
  ...
  componentDidMount() {
    ...
    const height = _props.height,
          ...
          props = objectWithoutProperties(_props, ['height', 'width', 'children']);
    ...
    pdf(this.container).toBlob().then(blob => {
  ...
  componentDidUpdate() {
    ...
    const height = _props2.height,
          width = _props2.width,
          children = _props2.children,
          props = objectWithoutProperties(_props2, ['height', 'width', 'children']);
